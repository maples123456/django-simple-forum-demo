...
function ChartTooltip({ active, payload, label }) {
  ...
  const percentKeys = new Set(['dauWau', 'dauMau', 'activationRate', 'anonymousRegistrationRate', 'interactionRate', 'd1', 'd7'])
  return <div className="chart-tooltip"><strong>{label}</strong>{payload.map(item => <span key={item.dataKey || item.name} style={{ color: item.color || item.fill }}>{item.name}：{number(item.value)}{percentKeys.has(item.dataKey) ? '%' : ''}</span>)}</div>
}
...
function BehaviorTooltip({ active, payload, label }) {
  ...
  return <div className="chart-tooltip"><strong>{label}</strong><span>D1：{row.d1}%（{row.d1RetainedUsers}/{row.d1CohortSize}）</span><span>D7：{row.d7}%（{row.d7RetainedUsers}/{row.d7CohortSize}）</span><small>不同观察日使用各自已成熟 Cohort</small></div>
}
...
function AnalyticsDashboard() {
  ...
  const cards = [
    ...
    ['匿名 UV', summary.anonymousUv, '曾以未登录状态浏览'], ['匿名注册', summary.anonymousRegistrations, '同 anonymous_id 可归因'],
    ['匿名注册转化率', `${summary.anonymousRegistrationRate || 0}%`, '匿名注册 / 匿名 UV'], ['新增激活率', `${summary.activationRate || 0}%`, '注册日有效活跃 / 新增'],
    ['人均帖子浏览', summary.postViewDepth, '帖子浏览 / 活跃用户'], ['创作者率', `${summary.creatorRate || 0}%`, '发帖用户 / 活跃用户'],
    ['讨论参与率', `${summary.discussionRate || 0}%`, '回复用户 / 活跃用户'], ['D1 留存', `${summary.d1Retention || 0}%`, '成熟注册 Cohort'], ['D7 留存', `${summary.d7Retention || 0}%`, '成熟注册 Cohort'], ['创作者 D7 留存', `${summary.creatorD7Retention || 0}%`, `注册首日创作者 · n=${summary.creatorD7CohortSize || 0}`],
  ]
  const likeCards = [
  ...
  const participationCards = [
    ['发帖用户', summary.postingUsers, `窗口内按 user_id 去重`],
    ['回复用户', summary.replyingUsers, `窗口内按 user_id 去重`],
    ['点赞用户', summary.likingUsers, `窗口内按 user_id 去重`],
  ]
  const trend = data?.trend || []
  const segments = (data?.segments || []).filter(item => item.value > 0)
  const contentHealth = data?.contentHealth || {}
  const healthCards = [
    ['窗口新帖', contentHealth.postCount, '发帖日在所选窗口'],
    ['每帖回复数', contentHealth.repliesPerPost ?? '—', '回复总数 / 全部新帖'],
    ['零回复帖子率', contentHealth.zeroReplyRate == null ? '—' : `${contentHealth.zeroReplyRate}%`, '截至窗口结束'],
    ['首次回复中位数', contentHealth.medianFirstReplyHours == null ? '—' : `${contentHealth.medianFirstReplyHours}h`, '仅统计已获回复帖'],
  ]
  return <main className="analytics-shell"><header><div><span>ADMIN ANALYTICS</span><h1>论坛业务观测台</h1><p>从用户规模、内容消费、内容生产、互动质量与留存观察社区健康度。</p></div><a href="/">返回论坛</a></header>
    ...
    <section className="chart-grid">
      ...
      <article className="analytics-card chart-wide"><h2>匿名 UV、注册转化与新增激活</h2><p>匿名注册只接受同一 anonymous_id 的先浏览后注册归因；激活要求注册当天产生有效论坛行为。</p><div className="chart-box"><ResponsiveContainer><ComposedChart data={trend} margin={{ top: 16, right: 18, left: -10 }}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="date" tickFormatter={shortDate} minTickGap={24}/><YAxis yAxisId="count"/><YAxis yAxisId="rate" orientation="right" unit="%"/><Tooltip content={<ChartTooltip/>}/><Legend/><Bar name="匿名 UV" yAxisId="count" dataKey="anonymousUv" fill={chartColors[1]}/><Bar name="匿名注册" yAxisId="count" dataKey="anonymousRegistrations" fill={chartColors[2]}/><Line name="匿名注册转化率" yAxisId="rate" type="monotone" dataKey="anonymousRegistrationRate" stroke={chartColors[3]} strokeWidth={2}/><Line name="新增激活率" yAxisId="rate" type="monotone" dataKey="activationRate" stroke={chartColors[0]} strokeWidth={3}/></ComposedChart></ResponsiveContainer></div></article>
      <article className="analytics-card chart-wide"><h2>内容生产与互动用户</h2><p>卡片是窗口去重用户，折线是每日去重用户；不能把每日用户数求和作为窗口用户数。同一用户可以同时属于三类。</p><div className="definition-metrics three-metrics">{participationCards.map(([label, value, note]) => <div key={label}><span>{label}</span><strong>{value ?? 0}</strong><small>{note}</small></div>)}</div><div className="chart-box compact-chart"><ResponsiveContainer><LineChart data={trend} margin={{ top: 16, right: 18, left: -10 }}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="date" tickFormatter={shortDate} minTickGap={24}/><YAxis/><Tooltip content={<ChartTooltip/>}/><Legend/><Line name="发帖用户" dataKey="postingUsers" stroke={chartColors[0]} strokeWidth={3} dot={false}/><Line name="回复用户" dataKey="replyingUsers" stroke={chartColors[1]} strokeWidth={3} dot={false}/><Line name="点赞用户" dataKey="likingUsers" stroke={chartColors[2]} strokeWidth={3} dot={false}/></LineChart></ResponsiveContainer></div></article>
      ...
      <article className="analytics-card chart-wide"><h2>新帖回应质量</h2><p>以窗口内新建帖子为 Cohort，观察截至窗口结束；越靠近窗口末尾的帖子观察时间越短。</p><div className="definition-metrics">{healthCards.map(([label, value, note]) => <div key={label}><span>{label}</span><strong>{value ?? 0}</strong><small>{note}</small></div>)}</div></article>
      ...
      <article className="analytics-card"><h2>板块用户参与</h2><p>各板块独立按 user_id 去重；同一用户访问多个板块时会分别出现。</p><div className="chart-box chart-tall"><ResponsiveContainer><BarChart data={data?.boards || []} layout="vertical" margin={{ top: 10, right: 12, left: 18 }}><CartesianGrid strokeDasharray="3 3" horizontal={false}/><XAxis type="number"/><YAxis dataKey="name" type="category" width={76}/><Tooltip content={<ChartTooltip/>}/><Legend/><Bar name="板块活跃用户" dataKey="activeUsers" fill={chartColors[0]}/><Bar name="浏览用户" dataKey="viewUsers" fill={chartColors[4]}/><Bar name="互动用户" dataKey="interactionUsers" fill={chartColors[1]}/><Bar name="发帖用户" dataKey="postingUsers" fill={chartColors[2]}/></BarChart></ResponsiveContainer></div></article>
      ...
      <article className="analytics-card chart-wide"><h2>首日行为与后续留存</h2><p>按注册首日最深行为互斥分层，比较成熟 Cohort 的加权 D1/D7 exact-day 留存；这是关联分析，不代表行为导致留存。悬停可查看实际分子与分母，避免忽略小样本。</p><div className="chart-box compact-chart"><ResponsiveContainer><BarChart data={data?.behaviorRetention || []} margin={{ top: 16, right: 18, left: -10 }}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name"/><YAxis unit="%"/><Tooltip content={<BehaviorTooltip/>}/><Legend/><Bar name="D1 留存" dataKey="d1" fill={chartColors[0]} radius={[4,4,0,0]}/><Bar name="D7 留存" dataKey="d7" fill={chartColors[1]} radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></div></article>
